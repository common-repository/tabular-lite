...
class WPGO_Tabular_Options {
	...
	/**
	 * Register Tabular options with Settings API.
	 *
	 * @since 0.1.0
	 */
	public function register_settings() {
		...
		register_setting(
			'wpgo_tabular_options_group',
			'wpgo_tabular_options',
	...
	public function sanitize_options( $input ) {
		...
		/* Sanitize tabular options via filter hook. */
	...
	public static function get_options() {
		...
		$options = get_option( 'wpgo_tabular_options' );
	...
	public function render_plugin_form() {
		?>
		<div class="wrap">
			<h2 class="plugin-title">Tabular Settings</h2>

			<!-- Start Main Form -->
			<form id="plugin-options-form" method="post" action="options.php">
				<?php settings_fields( 'wpgo_tabular_options_group' ); ?>
				<?php $options = self::get_options(); ?>

				<table class="form-table">

					<tr valign="top">
						<th scope="row">Tabular Archives</th>
						<td>
							<label><input name="wpgo_tabular_options[chk_post_content]" type="checkbox" value="1" <?php if ( isset( $options['chk_post_content'] ) ) {
	...
	public function add_options_page() {
		...
		add_submenu_page(
			...
			__( 'Tabular Settings', 'wpgo-tabular' ),
			__( 'Settings', 'wpgo-tabular' ),
			'manage_options',
			'wpgo-tabular-settings-page',
